...
def calc_y_for_regression(arousals, valences):
...
def build_dataset_regression(datapath, generative_model, char2idx, layer_idx):
    xs, arousals, valences = [], [], []
    ...
    for row in data:
        ...
        data_dir = os.path.dirname(datapath)
        data_dir = data_dir
        phrase_path = os.path.join(data_dir, filepath).replace('.mid', '') + ".mid"
        encoded_path = os.path.join(data_dir, filepath).replace('.mid', '') + ".npy"
    ...
    ys = calc_y_for_regression(np.array(arousals).astype(np.float), np.array(valences).astype(np.float))
    return np.array(xs), np.array(ys)
...
def get_activated_neurons(sent_classfier):
    ...
    return neuron_ixs
    # return np.random.choice(256, 30, replace=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--regression', type=int, deafult=True, help="To use a regression or classifier.")
    ...
    x, y = build_dataset_regression(opt.train, generative_model, char2idx, opt.cellix)

    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=42)
    train_dataset, test_dataset = (x_train, y_train), (x_test, y_test)

    # Train model
    sentneuron_ixs, score = train_classifier_model_regression(train_dataset, test_dataset)
